...
def main():
    ...
    for count_i, dataset_idx in enumerate(indices):
        ...
        # Stable path: copy BEV to host and feed C.
        # PT_PARITY_NO_D_NO_QMEM=True makes TRT match the current PyTorch trace:
        # raw/head BEV for every frame, no D fused BEV into the head.
        if PT_PARITY_NO_D_NO_QMEM or scene_is_first_frame:
            b_out = B.copy_outputs_to_host()
            head_bev = np.ascontiguousarray(b_out["bev_features"].astype(np.float32))
            if head_bev.ndim == 3:
                head_bev = head_bev[None, ...]
        else:
            d_out = D.copy_outputs_to_host()
            head_bev = np.ascontiguousarray(d_out["fused_bev"].astype(np.float32))
            if head_bev.ndim == 3:
                head_bev = head_bev[None, ...]
        ...
        for step_i, t in enumerate(timesteps[:-1]):
            ...
            if count_i in (0, 1):
                with open(dbg_e2e / f"04_C_step{step_i:02d}_engine.txt", "w") as f:
                    f.write("C_first\n" if scene_is_first_frame else "C_temporal\n")
            ...
            if count_i == 0:
                ...
                with open(dbg_dir / f"step_{step_i:02d}_engine.txt", "w") as f:
                    f.write("C_first\n" if scene_is_first_frame else "C_temporal\n")
                print(f"[debug save] count_i={count_i} dataset_idx={dataset_idx} step={step_i} engine={'C_first' if scene_is_first_frame else 'C_temporal'} dir={dbg_dir}")
