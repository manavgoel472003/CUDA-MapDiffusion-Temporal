...
ENGINE_C_FIRST = ROOT / "model/mapdiffusion_temporal_routeB/build/mapdiffusion.temporal_head.manual_tq.firstframe.opset13.fp32.plan"
...
def format_result(line_preds, cls_logits, score_thr=0.0):
    """Convert final TRT head outputs to metric vector submission format.

    For parity with PyTorch MapDiffusion output, do NOT filter by prop_mask.
    PyTorch prop_mask is all False for the 20-sample reference but still outputs
    all 100 predictions, so it is not a valid-output filter.
    """
    ...
    result = {
        "vectors": [vectors[i] for i in idxs],
        "scores": [float(scores[i]) for i in idxs],
        "labels": [int(labels[i]) for i in idxs],

        # PyTorch reference shows prop_mask true 0/100 while still outputting all
        # 100 predictions. Keep this field for structural parity, but do not use
        # it for filtering.
        "prop": [False for _ in idxs],
        "prop_mask": [False for _ in idxs],
    }
...
def ddim_update_query_like_pytorch(
    ...
    query_next = query_next.astype(np.float32)
...
def _as_rot3x3(rot):
    ...
    if arr.size == 4:
        # nuScenes quaternion order is usually [w, x, y, z]
        w, x, y, z = arr
    ...
    raise ValueError(f"Unsupported rotation shape/value: shape={np.asarray(rot).shape}")

def _as_trans3(trans):
    return np.asarray(trans, dtype=np.float64).reshape(-1)[:3]

def warp_prev_bev_like_pytorch(prev_bev_np, prev_meta, curr_meta, roi_size=(60.0, 30.0)):
    """Match MapDiffusionTemporal.update_bev_feature() BEV memory warp."""
    prev_meta = _meta_scalar(prev_meta)
    curr_meta = _meta_scalar(curr_meta)

    if prev_meta is None or curr_meta is None:
        return np.ascontiguousarray(prev_bev_np.astype(np.float32))

    prev_bev_np = np.ascontiguousarray(prev_bev_np.astype(np.float32))
    if prev_bev_np.ndim == 3:
        prev_bev_np = prev_bev_np[None, ...]

    _, c, bev_h, bev_w = prev_bev_np.shape

    device = torch.device("cpu")

    xmin, xmax = -roi_size[0] / 2.0, roi_size[0] / 2.0
    ymin, ymax = -roi_size[1] / 2.0, roi_size[1] / 2.0

    x = torch.linspace(xmin, xmax, bev_w, dtype=torch.float64, device=device)
    y = torch.linspace(ymax, ymin, bev_h, dtype=torch.float64, device=device)

    # Keep original repo behavior: y, x = torch.meshgrid(y, x)
    yy, xx = torch.meshgrid(y, x)

    z = torch.zeros_like(xx)
    ones = torch.ones_like(xx)
    plane = torch.stack([xx, yy, z, ones], dim=-1)

    prev_e2g_trans = torch.as_tensor(
        _as_trans3(prev_meta["ego2global_translation"]),
        dtype=torch.float64,
        device=device,
    )
    prev_e2g_rot = torch.as_tensor(
        _as_rot3x3(prev_meta["ego2global_rotation"]),
        dtype=torch.float64,
        device=device,
    )
    curr_e2g_trans = torch.as_tensor(
        _as_trans3(curr_meta["ego2global_translation"]),
        dtype=torch.float64,
        device=device,
    )
    curr_e2g_rot = torch.as_tensor(
        _as_rot3x3(curr_meta["ego2global_rotation"]),
        dtype=torch.float64,
        device=device,
    )

    prev_g2e_matrix = torch.eye(4, dtype=torch.float64, device=device)
    prev_g2e_matrix[:3, :3] = prev_e2g_rot.T
    prev_g2e_matrix[:3, 3] = -(prev_e2g_rot.T @ prev_e2g_trans)

    curr_e2g_matrix = torch.eye(4, dtype=torch.float64, device=device)
    curr_e2g_matrix[:3, :3] = curr_e2g_rot
    curr_e2g_matrix[:3, 3] = curr_e2g_trans

    curr2prev_matrix = prev_g2e_matrix @ curr_e2g_matrix

    prev_coord = torch.einsum("lk,ijk->ijl", curr2prev_matrix, plane).float()[..., :2]
    prev_coord[..., 0] = prev_coord[..., 0] / (roi_size[0] / 2.0)
    prev_coord[..., 1] = -prev_coord[..., 1] / (roi_size[1] / 2.0)

    prev_bev = torch.from_numpy(prev_bev_np[0]).to(device=device, dtype=torch.float32)

    warped_feat = F.grid_sample(
        prev_bev.unsqueeze(0),
        prev_coord.unsqueeze(0),
        padding_mode="zeros",
        align_corners=False,
    ).squeeze(0)

    return np.ascontiguousarray(warped_feat.unsqueeze(0).cpu().numpy().astype(np.float32))


def main():
    PT_PARITY_NO_D_NO_QMEM = os.environ.get('PT_PARITY_NO_D_NO_QMEM', '0') == '1'
    PT_QUERY_REPLAY = os.environ.get('PT_QUERY_REPLAY', '0') == '1'
    PT_BEV_REPLAY = os.environ.get('PT_BEV_REPLAY', '0') == '1'
    PT_PREVQ_REPLAY = os.environ.get('PT_PREVQ_REPLAY', '0') == '1'
    PT_QUERY_TRACE_ROOT = Path(os.environ.get(
        'PT_QUERY_TRACE_ROOT',
        '/home/018198687/Mapping/Lidar_AI_Solution/CUDA-BEVFusion/model/mapdiffusion_temporal_routeB/pytorch_working_trace_2',
    ))

    print('PT_PARITY_NO_D_NO_QMEM:', PT_PARITY_NO_D_NO_QMEM)
    print('PT_QUERY_REPLAY:', PT_QUERY_REPLAY)
    print('PT_BEV_REPLAY:', PT_BEV_REPLAY)
    print('PT_PREVQ_REPLAY:', PT_PREVQ_REPLAY)
    print('PT_QUERY_TRACE_ROOT:', PT_QUERY_TRACE_ROOT)
    ...
    print("ENGINE_C_FIRST:", ENGINE_C_FIRST)
    print("=" * 100)
    ...
    engine_c_first = load_engine(ENGINE_C_FIRST, logger)
    ...
    C_first = TrtModule("EngineC_MapDiffusionHead_FirstFrame", engine_c_first)
    ...
    for count_i, dataset_idx in enumerate(indices):
        ...
        if count_i in (0, 1):
            dbg_e2e = out_dir / f"debug_e2e_idx{dataset_idx:06d}"
            dbg_e2e.mkdir(parents=True, exist_ok=True)
            save_np_debug(dbg_e2e / "00_img.npy", img)
            save_np_debug(dbg_e2e / "00_ego2img.npy", ego2img)
            with open(dbg_e2e / "meta.txt", "w") as f:
                f.write(f"count_i={count_i}\n")
                f.write(f"dataset_idx={dataset_idx}\n")
                f.write(f"token={token}\n")
                f.write(f"scene={scene}\n")
                f.write(f"new_scene={new_scene}\n")
        ...
        if count_i in (0, 1):
            a_dbg = A.copy_outputs_to_host()
            save_np_debug(dbg_e2e / "01_A_feat0.npy", a_dbg["feat0"])
            save_np_debug(dbg_e2e / "01_A_feat1.npy", a_dbg["feat1"])
            save_np_debug(dbg_e2e / "01_A_feat2.npy", a_dbg["feat2"])
            b_dbg = B.copy_outputs_to_host()
            save_np_debug(dbg_e2e / "02_B_raw_bev.npy", b_dbg["bev_features"])

        # First frame in a scene has no valid temporal/query memory.
        # Use this flag later to choose the first-frame C engine.
        scene_is_first_frame = prev_bev_state is None

        if scene_is_first_frame:
            ...
            # PyTorch update_bev_feature() warps previous fused BEV into the
            # current ego frame before passing it to stream_fusion_neck.
            warped_prev_bev = warp_prev_bev_like_pytorch(
                prev_bev_state,
                prev_bev_meta_state,
                meta,
                roi_size=(60.0, 30.0),
            )
            D.bind_host_input("prev_bev", warped_prev_bev)
            if count_i in (0, 1):
                save_np_debug(dbg_e2e / "02_D_prev_bev_warped_input.npy", warped_prev_bev)
                print(
                    "[D_PREV_WARP]",
                    "idx=", dataset_idx,
                    "mean=", float(warped_prev_bev.mean()),
                    "std=", float(warped_prev_bev.std()),
                    "min=", float(warped_prev_bev.min()),
                    "max=", float(warped_prev_bev.max()),
                )
        ...
        if count_i in (0, 1):
            d_dbg = D.copy_outputs_to_host()
            save_np_debug(dbg_e2e / "03_D_fused_bev.npy", d_dbg["fused_bev"])

        # Working PyTorch path uses the temporal neck / D fused BEV as the
        # head input, even for the first frame. Do NOT replace this with raw B.
        ...
        # Bind C/C_first inputs inside the diffusion loop after selecting engine.
        ...
        if PT_PARITY_NO_D_NO_QMEM:
            prev_query_feat_input = np.zeros((1, 100, 512), dtype=np.float32)
        else:
        ...
        if PT_PREVQ_REPLAY:
            pt_prev_path = (
                PT_QUERY_TRACE_ROOT
                / f"debug_e2e_idx{dataset_idx:06d}"
                / "04_C_step00_input_prev_query_feat.npy"
            )
            if not pt_prev_path.exists():
                raise FileNotFoundError(f"PT_PREVQ_REPLAY missing prev query feat: {pt_prev_path}")
            prev_query_feat_input = np.ascontiguousarray(np.load(pt_prev_path).astype(np.float32))
            print(f"[PT_PREVQ_REPLAY] idx={dataset_idx} loaded {pt_prev_path}")

        print(
            "[BEV_STATE_BEFORE_C]",
            "count_i=", count_i,
            "dataset_idx=", dataset_idx,
            "scene_first=", scene_is_first_frame,
            "head_bev_shape=", getattr(head_bev, "shape", None),
            "head_bev_mean=", float(head_bev.mean()),
            "head_bev_std=", float(head_bev.std()),
            "head_bev_min=", float(head_bev.min()),
            "head_bev_max=", float(head_bev.max()),
        )
        ...
        for step_i, t in enumerate(timesteps[:-1]):
            ...
            C_run = C_first if (scene_is_first_frame or PT_PARITY_NO_D_NO_QMEM) else C
            if PT_PARITY_NO_D_NO_QMEM:
                C_run = C_first

            if count_i == 0:
                dbg_dir = out_dir / "debug_c_inputs_idx0"
                dbg_dir.mkdir(parents=True, exist_ok=True)
                np.save(dbg_dir / f"step_{step_i:02d}_bev_features.npy", head_bev.astype(np.float32))
                np.save(dbg_dir / f"step_{step_i:02d}_query_coords.npy", query_coords.astype(np.float32))
                np.save(dbg_dir / f"step_{step_i:02d}_timestep.npy", t_arr.astype(np.float32))
                np.save(dbg_dir / f"step_{step_i:02d}_prev_query_feat.npy", prev_query_feat_input.astype(np.float32))
                with open(dbg_dir / f"step_{step_i:02d}_engine.txt", "w") as f:
                    f.write("C_first\n" if C_run is C_first else "C_temporal\n")
                print(f"[debug save] count_i={count_i} dataset_idx={dataset_idx} step={step_i} engine={'C_first' if C_run is C_first else 'C_temporal'} dir={dbg_dir}")

            # Optional diagnostic mode: use exact PyTorch query_coords for this
            # dataset idx and diffusion step. This isolates C-engine parity from
            # stochastic DDIM/query rollout mismatch.
            if PT_QUERY_REPLAY:
                pt_q_path = (
                    PT_QUERY_TRACE_ROOT
                    / f"debug_e2e_idx{dataset_idx:06d}"
                    / f"04_C_step{step_i:02d}_input_query_coords.npy"
                )
                if not pt_q_path.exists():
                    raise FileNotFoundError(f"PT_QUERY_REPLAY missing query coords: {pt_q_path}")
                query_coords = np.ascontiguousarray(np.load(pt_q_path).astype(np.float32))
                print(f"[PT_QUERY_REPLAY] idx={dataset_idx} step={step_i} loaded {pt_q_path}")

            # Optional diagnostic mode: use exact PyTorch C input BEV.
            # This isolates C-engine/wrapper parity from A/B/D temporal BEV drift.
            if PT_BEV_REPLAY:
                pt_bev_path = (
                    PT_QUERY_TRACE_ROOT
                    / f"debug_e2e_idx{dataset_idx:06d}"
                    / f"04_C_step{step_i:02d}_input_bev.npy"
                )
                if not pt_bev_path.exists():
                    raise FileNotFoundError(f"PT_BEV_REPLAY missing BEV: {pt_bev_path}")
                head_bev = np.ascontiguousarray(np.load(pt_bev_path).astype(np.float32))
                print(
                    f"[PT_BEV_REPLAY] idx={dataset_idx} step={step_i} "
                    f"loaded {pt_bev_path} "
                    f"shape={head_bev.shape} "
                    f"mean={float(head_bev.mean()):.8f} "
                    f"min={float(head_bev.min()):.8f} "
                    f"max={float(head_bev.max()):.8f}"
                )

            # Save the ACTUAL inputs that will be bound to C, after replay overrides.
            if count_i in (0, 1):
                with open(dbg_e2e / f"04_C_step{step_i:02d}_engine.txt", "w") as f:
                    f.write("C_first\n" if C_run is C_first else "C_temporal\n")
                save_np_debug(dbg_e2e / f"04_C_step{step_i:02d}_input_bev.npy", head_bev)
                save_np_debug(dbg_e2e / f"04_C_step{step_i:02d}_input_query_coords.npy", query_coords)
                save_np_debug(dbg_e2e / f"04_C_step{step_i:02d}_input_timestep.npy", t_arr)
                save_np_debug(dbg_e2e / f"04_C_step{step_i:02d}_input_prev_query_feat.npy", prev_query_feat_input)

            # Safer replay-style binding: bind only the selected engine,
            # and allocate outputs after all current inputs are bound.
            C_run.bind_host_input("bev_features", head_bev)
            C_run.bind_host_input("query_coords", np.ascontiguousarray(query_coords.astype(np.float32)))
            C_run.bind_host_input("timestep", np.ascontiguousarray(t_arr.astype(np.float32)))

            if not scene_is_first_frame:
                C_run.bind_host_input("prev_query_feat", prev_query_feat_input)
            ...
            if count_i in (0, 1):
                save_np_debug(dbg_e2e / f"05_C_step{step_i:02d}_output_line_preds.npy", line_preds)
                save_np_debug(dbg_e2e / f"05_C_step{step_i:02d}_output_cls_logits.npy", cls_logits)
                if "query_feat" in out:
                    save_np_debug(dbg_e2e / f"05_C_step{step_i:02d}_output_query_feat.npy", out["query_feat"])
        ...
        prev_bev_meta_state = meta
        if not PT_PARITY_NO_D_NO_QMEM:
