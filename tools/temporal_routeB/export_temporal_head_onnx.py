...
EXPORT_FIRST_FRAME = os.environ.get("EXPORT_FIRST_FRAME", "0") == "1"
...
class ManualTemporalQueryFusion(nn.Module):
    ...
    def forward(self, current_query, prev_query=None, timestep=None, prev_valid=None):
        # Match original TemporalQueryFusion semantics:
        # - no previous query -> bypass
        # - prev_valid all false -> bypass
        # This is critical for first frame / scene reset.
        if prev_query is None:
            return current_query

        if prev_valid is not None:
            valid = prev_valid.to(device=current_query.device)

            # Export wrapper may pass [B,100] float mask; original module often
            # uses [B]. Collapse any per-query mask to per-batch validity.
            if valid.dim() > 1:
                valid = valid.reshape(valid.shape[0], -1).any(dim=1)

            valid = valid.bool().view(-1)
...
def main():
    ...
    if EXPORT_FIRST_FRAME:
        # First-frame / scene-reset export: bypass temporal query fusion.
        prev_query_valid = torch.zeros((1, 100), dtype=torch.float32, device="cuda")
        print("EXPORT_FIRST_FRAME=True -> prev_query_valid all zeros")
    else:
        # Active temporal export: keep temporal query fusion in graph.
        prev_query_valid = torch.ones((1, 100), dtype=torch.float32, device="cuda")
        print("EXPORT_FIRST_FRAME=False -> prev_query_valid all ones")
